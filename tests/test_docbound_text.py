...
from levanter.data.docbound_text import SplitGenDataset, TokenSeqWithDocBoundsDataset, make_examples, make_skip_mask
...
def test_skip_mask():
    ...
    seq_len = 16
    skip_mask = make_skip_mask(start_inds, seq_len, skip_after_k_tokens)

    for e, t in zip(skip_mask, target):
        assert e == t, (skip_mask.astype(int), target.astype(int))
